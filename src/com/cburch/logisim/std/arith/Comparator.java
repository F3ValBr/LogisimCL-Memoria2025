...
public class Comparator extends InstanceFactory {
    ...
    public void propagate(InstanceState state) {
        ...
        boolean strict = Boolean.TRUE.equals(state.getAttributeValue(ATTR_STRICT_EQ));
        ...
        for (int pos = maxlen - 1; pos >= 0; pos--) {
            ...
            // Errores: propagan error siempre
            ...
            if (!strict) {
                // Modo clásico: X fuerza UNKNOWN global
                ...
                // Modo igualdad estricta:
                // - X vs X → no decide nada (continúa)
                // - X vs 0/1 → desigual seguro; orden indeterminado
                if (ab == Value.UNKNOWN || bb == Value.UNKNOWN) {
                    if (ab != bb) { // uno es X y el otro 0/1
                        eq = Value.FALSE;
                        gt = Value.UNKNOWN;
                        lt = Value.UNKNOWN;
                        break;
                    } else {
                        // X vs X: seguimos mirando bits más bajos
                        continue;
                    }
                }
                // Ambos conocidos:
            ...
            // Si son iguales en este bit, seguimos al siguiente más bajo
        }
    ...
    public void paintInstance(InstancePainter painter) {
        boolean strict = painter.getAttributeValue(ATTR_STRICT_EQ);

        painter.drawBounds();
        painter.drawPort(IN0);
        painter.drawPort(IN1);
        painter.drawPort(GT, ">", Direction.WEST);
        painter.drawPort(EQ, strict ? "==" : "=", Direction.WEST);
